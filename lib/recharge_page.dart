  ...
  void _onConfirmRecharge() {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      if (mounted) {
        ...
        // Add amount to current balance and sync back
        double newBalance = widget.currentBalance + rechargeAmount;
        widget.onBalanceUpdated(newBalance);
        
        _showSuccessDialog(rechargeAmount, newBalance);
      }
    });
  }

  void _showSuccessDialog(double addedAmount, double updatedBalance) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text("Successfully added: ${addedAmount.toStringAsFixed(2)} ETB"),
            const SizedBox(height: 8),
            Text("New Balance: ${updatedBalance.toStringAsFixed(2)} ETB", 
              style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF00796B))
            ),
          ],
        ),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(
                        "${widget.currentBalance.toStringAsFixed(2)} ETB",
                        style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold),
                      ),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text("Telebirr Wallet (Default)", style: TextStyle(fontWeight: FontWeight.bold)),
                            Text("Balance: ${widget.currentBalance.toStringAsFixed(2)}", style: const TextStyle(color: Colors.grey, fontSize: 12)),
                          ],
                        ),
                      ),
                      const Icon(Icons.check_circle, color: Color(0xFF00796B)),
